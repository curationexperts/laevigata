  ...
  class MigrationMapper < Darlingtonia::MetadataMapper
    ...
    def date_created
      date_str =
        repository_object
          .archive_foxml
          .xpath('./foxml:digitalObject/foxml:objectProperties/foxml:property[@NAME="info:fedora/fedora-system:def/model#createdDate"]', NAMESPACES)
          .attribute('VALUE')
          .content

      date_str.present? && Array(Date.parse(date_str))
    end
