  ...
  module Workflow
    ...
    class SixtyDayEmbargoNotification
      ...
      def self.send_notification(work_id)
        notification = SixtyDayEmbargoNotification.new(work_id)
        notification.call
        notification
      end
      ...
      def subject
        "Embargo on #{@work.title.first} will expire in sixty days"
      end

      def message
        "Dear #{@user.display_name},\n\n" \
        "The embargo on your dissertation or thesis, #{@work.title.first}, will expire in sixty days.\n" \
