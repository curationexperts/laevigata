...
module ProquestBehaviors
  ...
  def mce_to_proquest(tinymce_output)
    clean_html = Sanitize.clean(tinymce_output, elements: ['p', 'em', 'b'])
    ...
    proquest_scrubber = Loofah::Scrubber.new do |node|
      ...
      node.name = "DISS_para" if node.name == "h1"
      node.name = "DISS_para" if node.name == "h2"
      node.name = "DISS_para" if node.name == "h3"
      node.name = "DISS_para" if node.name == "h4"
      node.name = "DISS_para" if node.name == "h5"
    end
