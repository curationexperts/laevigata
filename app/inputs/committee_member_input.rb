class CommitteeMemberInput < MultiValueInput
  # Overriding method from hydra-editor:
  # We are actually just displaying a single value for each
  # field of a committee member, so we override this method
  # so that there is only one value.  (e.g. each committee
  # member only has one name and one affiliation on the form.)
  def collection
    @collection ||= begin
