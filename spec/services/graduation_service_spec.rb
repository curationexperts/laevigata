...
describe GraduationService, :clean do
  ...
  let(:another_nongraduated_user) { FactoryBot.create(:another_nongraduated_user) }
  ...
  let(:graduated_etd_with_nongraduated_user) { FactoryBot.actor_create(:sample_data, degree: ["M.Div."], user: another_nongraduated_user) }
  ...
  before do
    ...
    subject = Hyrax::WorkflowActionInfo.new(graduated_etd_with_nongraduated_user, approving_user)
    sipity_workflow_action = PowerConverter.convert_to_sipity_action("approve", scope: subject.entity.workflow) { nil }
    Hyrax::Workflow::WorkflowActionService.run(subject: subject, action: sipity_workflow_action, comment: nil)
    expect(graduated_etd_with_nongraduated_user.to_sipity_entity.reload.workflow_state_name).to eq "approved"
  end
  ...
  describe "#run" do
    it "finds all works in an approved state that do not yet have a degree_awarded value" do
      ...
      expect(described_class.graduation_eligible_works.map(&:id)).to contain_exactly(graduated_etd.id, nongraduated_etd.id, graduated_etd_with_nongraduated_user.id)
    ...
    it "checks the graduation status for a given work" do
      ...
      expect(described_class.check_degree_status(graduated_etd_with_nongraduated_user)).to eq Date.new(2018, 1, 12)
      described_class.remove_instance_variable(:@registrar_data)
    end
